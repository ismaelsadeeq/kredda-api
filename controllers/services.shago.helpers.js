...
const startimesPurchase = async (user,trxRef,time,service,amount,cardNo,customerName,packageName,packageCode,period,addOnCode,addOnProductName,addOnAmount,res)=>{
  const wallet = await models.wallet.findOne(
    {
      where:{
        userId:user.id
